...
/*
    calculate_hash() {
        return SHA256(this.tx_sender + this.tx_receiver + this.amount).toString();
    }
    sign_tx(signing_key) {
        if (signing_key.getPublic('hex') != this.tx_sender){
            throw new Error("Your address doesn't match, you can't sign the transaction");
        }

        const hash_tx = this.calculate_hash();
        const signature = signing_key.sign(hash_tx, 'base64');
        this.signature = sign.toDER('hex');
    }
    // verify
    is_valid(){
        if (this.tx_sender == null ) return true;
